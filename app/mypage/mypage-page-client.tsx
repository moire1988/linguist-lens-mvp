...
function VocabCard({
  ...
  phrase: SavedPhrase;
  isArchived: boolean;
  onDelete: (id: string, expression: string) => void | Promise<void>;
  onArchive: (id: string, expression: string) => void | Promise<void>;
  onRestore: (id: string, expression: string) => void | Promise<void>;
}) {
  ...
  const handleArchiveClick = useCallback(async () => {
    ...
    try {
      await Promise.resolve(onArchive(phrase.id, phrase.expression));
    } finally {
      setArchiving(false);
    }
...
export function MypagePageClient({
  ...
  // ゲストは localStorage。ログイン済みは Supabase（取得は Server + 再同期）
  useEffect(() => {
    if (isSignedIn === undefined) return;
    const enrich = (list: SavedPhrase[]) =>
      list.map((p) =>
        p.example_translation
          ? p
          : {
              ...p,
              example_translation: examplesTranslationMap.get(
                p.expression.toLowerCase()
              ),
            }
      );
    if (!isSignedIn) {
      setVocabulary(enrich(getVocabulary()));
      return;
    }
    void getUserVocabularyAction().then((list) => setVocabulary(enrich(list)));
  }, [isSignedIn, examplesTranslationMap]);
  ...
  const handleDelete = useCallback(
    async (id: string, expression: string) => {
      if (isSignedIn) {
        const r = await deleteVocabularyAction(id);
        if (!r.success) {
          toast.error(r.error ?? "削除に失敗しました");
          return;
        }
        setVocabulary((prev) => prev.filter((p) => p.id !== id));
      } else {
  ...
  const handleArchive = useCallback(
    async (id: string, expression: string) => {
      if (isSignedIn) {
        const r = await archiveVocabularyAction(id);
        if (!r.success) {
  ...
  const handleRestore = useCallback(
    async (id: string, expression: string) => {
      if (isSignedIn) {
        const r = await restoreVocabularyAction(id);
        if (!r.success) {
          toast.error(r.error ?? "更新に失敗しました");
          return;
        }
  ...
  const handleClearByStatus = useCallback(async () => {
    if (isSignedIn) {
      const r = await clearVocabularyAction(statusTab);
      if (!r.success) {
        toast.error(r.error ?? "削除に失敗しました");
        return;
      }
      ...
      setVocabulary(clearByStatus(statusTab));
    }
